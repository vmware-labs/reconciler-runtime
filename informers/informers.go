...
func New(lease time.Duration, sm manager.SuperManager) Informers {
	return &informers{
		sm:    sm,
		lease: lease,
...
func (i *informer) getController(reconciler reconcile.Reconciler) (controller.Controller, error) {
	...
	if old := i.getAndSetInformer(reconciler, ctlr); old != nil {
		// TODO: tidy up ctlr.
		return old, nil
	}
...
func (i *informer) getAndSetInformer(reconciler reconcile.Reconciler, new controller.Controller) controller.Controller {
	i.m.Lock()
	defer i.m.Unlock()
